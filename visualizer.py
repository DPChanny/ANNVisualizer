...
def get_model(_hidden_layers):
    ...
    new_model.add(Dense(len(encoder.classes_), activation='softmax'))
    new_model.build([1, 2])
    return new_model


def process_image(_epoch):
    root_path = os.path.join(*[".", "results", str(HIDDEN_LAYERS)])
...
def main():
    ...
    pool.map_async(process_image, reversed(range(early_stopping.stopped_epoch - PATIENCE + 1))).get()
    pool.close()
